...
static struct file_operations gec2440buttons_fops = {
    .owner   =   THIS_MODULE,
...
static struct miscdevice gec2440_miscdev = {
	
    .minor = MISC_DYNAMIC_MINOR,
    .name = DEVICE_NAME,
    .fops = &gec2440buttons_fops,
};

/* device interface */
static int gec2440_buttons_probe(struct platform_device *pdev)
{
	struct resource *res;
	struct device *dev;
	int ret;
 ...
 /*get irq number*/
	...
	ret = misc_register(&gec2440_miscdev);
...
static int gec2440_buttons_remove(struct platform_device *dev)
{
	release_resource(buttons_mem);
	kfree(buttons_mem);
	buttons_mem = NULL;
	...
	iounmap(buttons_base);
	misc_deregister(&gec2440_miscdev);
	return 0;
}

/*平台驱动定义*/
static struct platform_driver gec2440buttons_driver = {
	.probe		= gec2440_buttons_probe,
	.remove		= gec2440_buttons_remove,
	.driver		= {
		.owner	= THIS_MODULE,
		.name	= "gec2440-buttons",
	},
};

static char banner[] __initdata =
	 "gec2440 Buttons Driver\n";

static int __init buttons_init(void)
{
	printk(banner);
	/*平台驱动注册*/
	platform_driver_register(&gec2440buttons_driver);
	return 0;
}

static void __exit buttons_exit(void)
{
	platform_driver_unregister(&gec2440buttons_driver);
}
...
MODULE_DESCRIPTION("gec2440 Buttons Driver");
MODULE_LICENSE("GPL");
